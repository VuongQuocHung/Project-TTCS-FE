...
import type { Branch, DashboardStats, LowStock } from "@/types/api";
...
export default function AdminBranchDashboardPage() {
  ...
  useEffect(() => {
    const fetchBranch = async () => {
      try {
        const data = await branchApi.getAdminById(branchId);
        setBranch(data);
      } catch {
        // ignore
      }
  ...
  const fetchDashboard = async () => {
    ...
    try {
      const [statsResponse, lowStockResponse] = await Promise.all([
        branchApi.getBranchDashboard(branchId, selectedMonth),
        branchApi.getBranchLowStock(branchId),
      ]);
      ...
      setError(apiError?.message || "Không thể tải dữ liệu chi nhánh.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter">
            Tổng quan: {branch?.name || `Chi nhánh #${branchId}`}
          </h1>
          {branch?.address && (
        ...
        <div className="flex items-center gap-3">
          <label className="sr-only" htmlFor="branch-dashboard-month">
            Chọn tháng thống kê
          </label>
          <input
