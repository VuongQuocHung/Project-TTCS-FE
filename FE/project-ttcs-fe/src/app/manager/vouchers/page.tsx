...
export default function ManagerVouchersPage() {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    try {
      const response = await voucherApi.getAllManager({ page, size: PAGE_SIZE });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await voucherApi.deleteManager(id);
      await fetchVouchers();
    } catch (err) {
      const apiError = err as ApiError;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingVoucher?.id) {
        await voucherApi.updateManager(editingVoucher.id, data);
      } else {
        await voucherApi.createManager(data);
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Voucher chi nhánh</h1>
        </div>
        <button
          type="button"
          ...
          <div className="w-full max-w-3xl rounded-2xl bg-white">
            <div className="flex items-center justify-between border-b border-slate-100 p-5">
              <h2 className="text-xl font-bold text-slate-900">
                {editingVoucher ? "ửa voucher" : "Thêm voucher"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="grid gap-4 p-5 md:grid-cols-2">
              ...
              <label className="space-y-1">
                <span className="text-sm font-medium text-slate-600">User ID riêng</span>
                <input
                  name="targetUserId"
                  type="number"
                  min="1"
                  defaultValue={formVoucher.targetUserId ?? ""}
                  placeholder="Bo trong neu dung chung"
                  className="w-full rounded-xl border border-slate-200 px-4 py-3"
                />
              </label>
